...
class SearchMovieCard extends StatelessWidget {
  final Movie movie;
  final MovieWatcherBloc movieWatcherBloc;

  const SearchMovieCard(
      {Key? key, required this.movie, required this.movieWatcherBloc})
      : super(key: key);
            ...
            onSelected: (MenuItem item) {
              if (item == MenuItem.favorite) {
                ...
                BlocProvider.of<MovieSearchBloc>(context)
                    .add(MovieSearchEvent.onRebuild(movie, false));
              } else {
                BlocProvider.of<MovieActorBloc>(context)
                    .add(AddHiddenMovie(movie));
                BlocProvider.of<MovieSearchBloc>(context)
                    .add(MovieSearchEvent.onRebuild(movie, true));
              }
              movieWatcherBloc.add(const MovieWatcherEvent.onFavorite());
            },
            icon: Container(
